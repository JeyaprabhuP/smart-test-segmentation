...
import {
  ...
  Users, LayoutGrid, List, BedDouble
} from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {vitals.map(v => (
          <motion.div key={v.label} whileHover={{ scale: 1.02 }} className={`bg-card rounded-xl border p-4 ${v.warn ? "border-critical/30 shadow-sm shadow-critical/10" : "border-border"}`}>
            <div className={`flex items-center gap-2 mb-1 text-xs font-medium ${v.warn ? "text-critical" : "text-muted-foreground"}`}>{v.icon} {v.label}</div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="vitals" className="mt-4">
          {patientHourly.length > 0 ? (
            ...
            <div className="text-center py-12 text-muted-foreground"><Clock className="w-10 h-10 mx-auto mb-3 opacity-40" /><p>No hourly vitals data available</p></div>
          )}
        </TabsContent>
        ...
        <TabsContent value="labs" className="mt-4">
          {patientLabs.length > 0 ? (
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{lab.category} · {lab.orderedBy} · {lab.orderedAt}</p>
                      </div>
                    </div>
                    <Badge variant={lab.status === "completed" ? "default" : "secondary"} className={lab.status === "completed" ? "bg-stable/10 text-stable border-stable/20" : ""}>{lab.status === "completed" ? "✓ Completed" : "⏳ Pending"}</Badge>
                  </div>
                  {lab.results.length > 0 && (
                    <Table>
                      <TableHeader><TableRow><TableHead><Microscope className="w-3.5 h-3.5 inline mr-1" />Parameter</TableHead><TableHead>Value</TableHead><TableHead>Normal Range</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
                      <TableBody>
                        {lab.results.map((r, i) => (
                          <TableRow key={i}>
                            ...
                            <TableCell>
                              <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${r.flag === "high" ? "bg-critical/10 text-critical" : r.flag === "low" ? "bg-moderate/10 text-moderate" : "bg-stable/10 text-stable"}`}>
                                {r.flag === "high" ? <TrendingUp className="w-3 h-3" /> : r.flag === "low" ? <TrendingDown className="w-3 h-3" /> : <Minus className="w-3 h-3" />}{r.flag.toUpperCase()}
                              </span>
                            </TableCell>
                          </TableRow>
                        ))}
            ...
            <div className="text-center py-12 text-muted-foreground"><FlaskConical className="w-10 h-10 mx-auto mb-3 opacity-40" /><p>No lab reports available</p></div>
          )}
        </TabsContent>
              ...
              {patient.history.map((h, i) => (
                <div key={i} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className={`w-3 h-3 rounded-full ${h.severity === "critical" ? "bg-critical" : h.severity === "moderate" ? "bg-moderate" : "bg-stable"}`} />
        ...
        <TabsContent value="tasks" className="mt-4">
          <div className="bg-card rounded-xl border border-border p-5">
            <div className="space-y-3">
              {tasks.map(t => (
                <motion.div key={t.id} whileHover={{ scale: 1.01 }} className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
      ...
      <div className="bg-accent/50 rounded-xl border border-secondary/20 p-5">
        <h3 className="font-semibold text-foreground mb-2 flex items-center gap-2"><Activity className="w-4 h-4 text-secondary" /> AI Triage Recommendation</h3>
        <p className="text-sm text-foreground">
          {patient.severity === "critical" ? "⚠️ Immediate intervention required. Multiple vitals exceeding critical thresholds." : patient.severity === "moderate" ? "⚡ Close monitoring advised. One or more vitals trending toward critical." : "✅ Patient stable. Continue standard monitoring protocol."}
        </p>
      </div>
    </div>
  );
...
const VitalChart = ({ data, dataKey, label, color, refLine }: { data: any[]; dataKey: string; label: string; color: string; refLine?: number }) => (
  <div className="bg-card rounded-xl border border-border p-4">
    <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-secondary" /> {label}</h4>
    <div className="h-40">
...
// ─── All Patients View (Enhanced) ───────────────────────────
const AllPatientsView = ({ onSelect }: { onSelect: (p: Patient) => void }) => {
  ...
  const doctors = [...new Set(patients.map(p => p.doctor).filter(d => d !== "—"))];
  ...
  const filtered = patients.filter(p => {
    ...
    return matchSearch && matchSeverity && matchStatus && matchWard && matchDoctor;
  }).sort((a, b) => {
    if (sortBy === "severity") { const o = { critical: 0, moderate: 1, stable: 2 }; return o[a.severity] - o[b.severity]; }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-foreground flex items-center gap-2"><Users className="w-6 h-6 text-secondary" /> All Patients</h1>
          <p className="text-muted-foreground text-sm mt-1">{filtered.length} of {patients.length} patients · All statuses</p>
        </div>
        <div className="flex gap-2">
          <Button variant={viewMode === "table" ? "default" : "outline"} size="icon" onClick={() => setViewMode("table")}><List className="w-4 h-4" /></Button>
          <Button variant={viewMode === "card" ? "default" : "outline"} size="icon" onClick={() => setViewMode("card")}><LayoutGrid className="w-4 h-4" /></Button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={severityFilter} onValueChange={setSeverityFilter}>
          <SelectTrigger className="w-[140px]"><AlertTriangle className="w-3.5 h-3.5 mr-1" /><SelectValue placeholder="Severity" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-[140px]"><BedDouble className="w-3.5 h-3.5 mr-1" /><SelectValue placeholder="Status" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={wardFilter} onValueChange={setWardFilter}>
          <SelectTrigger className="w-[140px]"><Activity className="w-3.5 h-3.5 mr-1" /><SelectValue placeholder="Ward" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={doctorFilter} onValueChange={setDoctorFilter}>
          <SelectTrigger className="w-[140px]"><Syringe className="w-3.5 h-3.5 mr-1" /><SelectValue placeholder="Doctor" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={sortBy} onValueChange={v => setSortBy(v as any)}>
          <SelectTrigger className="w-[130px]"><ArrowUpDown className="w-3.5 h-3.5 mr-1" /><SelectValue /></SelectTrigger>
          <SelectContent>
      ...
      {viewMode === "table" ? (
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead><BedDouble className="w-3.5 h-3.5 inline mr-1" />Status</TableHead>
                <TableHead><Heart className="w-3.5 h-3.5 inline mr-1" />HR</TableHead>
                <TableHead><Droplets className="w-3.5 h-3.5 inline mr-1" />O₂</TableHead>
                <TableHead></TableHead>
              </TableRow>
            ...
            <TableBody>
              {filtered.map(p => (
                <TableRow key={p.id} className="cursor-pointer hover:bg-muted/50" onClick={() => onSelect(p)}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-xs font-bold text-primary">{p.name.split(" ").map(n => n[0]).join("")}</div>
                      <div><span className="font-medium text-foreground">{p.name}</span><p className="text-xs text-muted-foreground">{p.age}y {p.gender}</p></div>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <Badge variant="outline" className={`text-xs ${
                      p.status === "admitted" ? "border-stable/40 text-stable" :
                      p.status === "waiting" ? "border-moderate/40 text-moderate" :
                      p.status === "discharged" ? "border-muted-foreground/40 text-muted-foreground" :
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(p => (
            <motion.div key={p.id} whileHover={{ scale: 1.01 }} onClick={() => onSelect(p)} className="bg-card rounded-xl border border-border p-4 cursor-pointer hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-sm font-bold text-primary">{p.name.split(" ").map(n => n[0]).join("")}</div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-foreground truncate">{p.name}</p>
                  <p className="text-xs text-muted-foreground">{p.age}y {p.gender} · {p.ward}</p>
                </div>
                <SeverityBadge severity={p.severity} />
              </div>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="bg-muted/50 rounded-lg p-2"><p className="text-[10px] text-muted-foreground">HR</p><p className={`text-sm font-bold ${p.vitals.heartRate > 100 ? "text-critical" : "text-foreground"}`}>{p.vitals.heartRate}</p></div>
                <div className="bg-muted/50 rounded-lg p-2"><p className="text-[10px] text-muted-foreground">O₂</p><p className={`text-sm font-bold ${p.vitals.oxygenSaturation < 92 ? "text-critical" : "text-foreground"}`}>{p.vitals.oxygenSaturation}%</p></div>
                <div className="bg-muted/50 rounded-lg p-2"><p className="text-[10px] text-muted-foreground">Status</p><p className="text-sm font-bold text-foreground capitalize">{p.status}</p></div>
              </div>
            </motion.div>
          ))}
        </div>
...
const ClinicianPatientList = ({ onSelect }: { onSelect: (p: Patient) => void }) => {
  ...
  const filtered = admittedPatients.filter(p => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Admitted patients · Click to view details</p>
      </div>
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          {["all", "critical", "moderate", "stable"].map(s => (
            ...
            >{s === "all" ? <Filter className="w-3.5 h-3.5 mr-1" /> : null}{s.charAt(0).toUpperCase() + s.slice(1)}</Button>
          ))}
        </div>
      </div>
      <div className="bg-card rounded-xl border border-border overflow-hidden">
            ...
            {filtered.map(p => {
              ...
              return (
                <TableRow key={p.id} className="cursor-pointer hover:bg-muted/50" onClick={() => onSelect(p)}>
                  ...
                  <TableCell className={`font-mono text-sm ${p.vitals.heartRate > 100 ? "text-critical font-bold" : ""}`}>{p.vitals.heartRate}</TableCell>
                  <TableCell className={`font-mono text-sm ${p.vitals.oxygenSaturation < 92 ? "text-critical font-bold" : ""}`}>{p.vitals.oxygenSaturation}%</TableCell>
                  <TableCell>{labCount > 0 && <Badge variant="outline" className="text-xs"><FlaskConical className="w-3 h-3 mr-1" />{labCount}</Badge>}</TableCell>
                  <TableCell><ChevronRight className="w-4 h-4 text-muted-foreground" /></TableCell>
                </TableRow>
              );
            })}
...
const ClinicianTaskList = () => {
  const allTasks = patients.filter(p => p.status === "admitted").flatMap(p => p.tasks.map(t => ({ ...t, patient: p.name, patientSeverity: p.severity })));
  const [tasks, setTasks] = useState(allTasks);
  const toggleTask = (id: string) => setTasks(prev => prev.map(t => t.id === id ? { ...t, done: !t.done } : t));
  const pending = tasks.filter(t => !t.done).sort((a, b) => { const order = { critical: 0, moderate: 1, stable: 2 }; return order[a.priority as keyof typeof order] - order[b.priority as keyof typeof order]; });
      ...
      <div className="space-y-2">
        {pending.map(t => (
          <motion.div key={t.id} whileHover={{ scale: 1.005 }} className="flex items-center gap-3 bg-card rounded-xl border border-border p-4">
            ...
            <div className="flex-1"><p className="text-sm font-medium text-foreground">{t.task}</p><p className="text-xs text-muted-foreground">{t.patient}</p></div>
            <SeverityBadge severity={t.priority} />
          </motion.div>
        ))}
      </div>
          ...
          <div className="space-y-2 opacity-60">
            {completed.map(t => (
              <div key={t.id} className="flex items-center gap-3 bg-card rounded-xl border border-border p-4">
                ...
                <div className="flex-1"><p className="text-sm font-medium text-foreground line-through">{t.task}</p><p className="text-xs text-muted-foreground">{t.patient}</p></div>
              </div>
            ))}
          </div>
        </>
...
const LabReportsPage = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input placeholder="Search tests, categories..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9" /></div>
        <div className="flex gap-2">
          {["all", "completed", "pending"].map(s => (
            <Button key={s} variant={statusFilter === s ? "default" : "outline"} size="sm" onClick={() => setStatusFilter(s)}>
              {s === "completed" && <CheckCircle2 className="w-3.5 h-3.5 mr-1" />}{s === "pending" && <Clock className="w-3.5 h-3.5 mr-1" />}{s.charAt(0).toUpperCase() + s.slice(1)}
            </Button>
          ))}
        </div>
      </div>
          ...
          <motion.div key={lab.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-xl border border-border p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-foreground flex items-center gap-2"><Beaker className="w-4 h-4 text-secondary" /> {lab.testName}</h4>
                  <p className="text-xs text-muted-foreground"><Pill className="w-3 h-3 inline mr-1" />{getPatientName(lab.patientId)} · {lab.category} · {lab.orderedBy}</p>
                </div>
              </div>
              <Badge variant={lab.status === "completed" ? "default" : "secondary"} className={lab.status === "completed" ? "bg-stable/10 text-stable border-stable/20" : ""}>{lab.status === "completed" ? "✓ Completed" : "⏳ Pending"}</Badge>
            </div>
...
const HourlyVitalsPage = () => {
  const admittedPatients = patients.filter(p => p.status === "admitted");
  const [selectedPatient, setSelectedPatient] = useState<string>(admittedPatients[0]?.id || "");
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {admittedPatients.map(p => (
          <Button key={p.id} variant={selectedPatient === p.id ? "default" : "outline"} size="sm" onClick={() => setSelectedPatient(p.id)} className="text-xs">
            <Heart className={`w-3 h-3 mr-1 ${p.severity === "critical" ? "text-critical" : p.severity === "moderate" ? "text-moderate" : "text-stable"}`} />{p.name.split(" ")[0]}
          </Button>
        ))}
      </div>
      {patient && patientData.length > 0 ? (
        <>
          <div className="flex items-center gap-3 p-4 bg-card rounded-xl border border-border">
            <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-sm font-bold text-primary">{patient.name.split(" ").map(n => n[0]).join("")}</div>
            <div><p className="font-semibold text-foreground">{patient.name}</p><p className="text-xs text-muted-foreground">{patient.ward} · {patient.bed} · {patient.doctor}</p></div>
            <SeverityBadge severity={patient.severity} />
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="text-center py-12 text-muted-foreground"><Clock className="w-10 h-10 mx-auto mb-3 opacity-40" /><p>No hourly data available</p></div>
      )}
    </div>
  );
};
...
const ClinicianDashboard = ({ page }: { page: string }) => {
  ...
  if (page === "all-patients") {
    ...
    return <AllPatientsView onSelect={setSelectedPatient} />;
  }
